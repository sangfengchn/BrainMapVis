...
subjects_list = [proj_dir '/info/subjects_trt.xlsx'];
...
%% loop subjects from HCP TRT release
data_dir = [hcp_dir '/TRT'];
        ...
        if ~exist(frest,'file')
            ...
            for idx_bd=1:num_bands
                ...
                disp(['alff and band-pass filtering: ' ...
                    bands_label{idx_bd} ' ...']) 
                [tmpalff,tmpalff_nor,tmpts_filt] = ccs_core_ampfilt(...
                    boldts_cc_lh(idx_mask_cc_lh,:)',...
                    rep_time,fbands{idx_bd}(1),fbands{idx_bd}(2),'true');
                alff_cc_lh = zeros(numel(mask_cc_lh),2);
                alff_cc_lh(idx_mask_cc_lh,:) = [tmpalff' tmpalff_nor'];
                ts_lh_filt = boldts_cc_lh';
                ts_lh_filt(:,idx_mask_cc_lh) = tmpts_filt;
                clear tmpalff tmpalff_nor tmpts_filt
                [tmpalff,tmpalff_nor,tmpts_filt] = ccs_core_ampfilt(...
                    boldts_cc_rh(idx_mask_cc_rh,:)',...
                    rep_time,fbands{idx_bd}(1),fbands{idx_bd}(2),'true');
                alff_cc_rh = zeros(numel(mask_cc_rh),2);
                alff_cc_rh(idx_mask_cc_rh,:) = [tmpalff' tmpalff_nor'];
